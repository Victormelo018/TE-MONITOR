...
POLL_INTERVAL = int(os.environ.get("POLL_INTERVAL", "5"))  # agent checa comando a cada 5s
...
def send_payload(payload):
    try:
        r = requests.post(f"{SERVER_URL}/api/collect", json=payload, headers=HEADERS, timeout=8)
        r.raise_for_status()
        return True
    except Exception as e:
